...
const howMuchPencil = (str) => {
  ...
  result.push(str);
  console.log(result);
  splitt.forEach((value,index) => {
    let slicced = str.slice(index + 1);
    result.push(slicced);
  });
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  recipeList.forEach(value => {
    let foods = value.split(' ');
    return foods[4] ? result.push(`${foods[2]} ${foods[3]} ${foods[4]}`) : foods[3] ? result.push(`${foods[2]} ${foods[3]}`)
      : result.push(foods[2]);
  });
  return result;
};
